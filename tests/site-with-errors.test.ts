        ...
        results.map(async ({issue: {url: issueUrl}}) => {
          ...
          return pollUntil(
            async () => {
              ...
              const prUrl = nodes
                .map(node => {
                  if ('source' in node && node.source?.url) return node.source.url
                  if ('subject' in node && node.subject?.url) return node.subject.url
                  return undefined
                })
                .find(url => !!url)
              if (!prUrl) throw new Error(`No linked PR found yet for issue ${issueUrl}`)
              const {owner: prOwner, repo: prRepo, pullNumber} =
