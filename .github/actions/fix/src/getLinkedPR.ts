...
export async function getLinkedPR(octokit: Octokit, { owner, repository, issueNumber }: Issue) {
  ...
  const pullRequest: { id: string; url: string; title: string } | undefined = timelineNodes
    .map((node) => {
